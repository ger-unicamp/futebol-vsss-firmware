...
float pid_computar(pid_config_t *pid, float setpoint_ticks, float valor_medido_ticks)
{
    ...
    if (I > pid->limite_max)
        pid->integral = pid->limite_max / pid->ki;
    else if (I < pid->limite_min)
        pid->integral = pid->limite_min / pid->ki;
    ...
    if (saida > pid->limite_max)
        saida = pid->limite_max;
    else if (saida < pid->limite_min)
        saida = pid->limite_min;
...
float autotune_computar(pid_autotune_t *tune, pid_config_t *pid, float setpoint_ticks, float valor_medido_ticks)
{
    ...
    if (valor_medido_ticks > tune->pico_max)
        tune->pico_max = valor_medido_ticks;
    if (valor_medido_ticks < tune->pico_min)
        tune->pico_min = valor_medido_ticks;
